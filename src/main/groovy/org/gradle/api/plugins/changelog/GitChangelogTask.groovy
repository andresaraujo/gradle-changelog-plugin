...
class GitChangelogTask extends DefaultTask {
    ...
    def changelog() {
        ...
        opts.to             = project.changelog.to ? project.changelog.to : getPreviousTag()

        commits = opts.from ? readGitLog(opts.grep, opts.from, opts.to) : readGitLog(opts.grep)

        println "Parsed ${commits.size()} commits"
        println "Generating changelog to ${opts.file} ( ${opts.version} )"
        writeChangelog(new RandomAccessFile(new File("$opts.file"), "rw"), commits, opts)
    }
